...
// ---------- header ----------
slide.addText([
  { text: "POS Product Team — H2 2026 KPIs", options: { fontSize: 23, bold: true, color: BLUE } },
], { x: 0.45, y: 0.28, w: 9.2, h: 0.42, fontFace: F, margin: 0, align: "left" });
slide.addText("Kurt Molina  ·  alignment w/ Ivan  ·  Jul 2026", {
  x: 9.7, y: 0.33, w: 3.18, h: 0.3, fontFace: F, fontSize: 9, color: GRAY, align: "right", margin: 0,
});
slide.addText(
  "North star: absolute PBT per line. Three tiers, phase-aware, delivery managed on business impact — per our 29 Jun call and the QuantumLight (Revolut) performance playbook.",
  { x: 0.45, y: 0.70, w: 12.43, h: 0.28, fontFace: F, fontSize: 10, color: INK, margin: 0 }
);
...
// Tier 0
slide.addText([
  { text: "TIER 0 · NORTH STAR — absolute PBT (P) per line", options: { fontSize: 9.5, bold: true, color: "FFFFFF", breakLine: true } },
...
slide.addText("PBT  =  Volume (disbursements)  ×  Profitability %", {
...
chip(c1x, y, 1.96, 0.42, BLUE_T, BLUE, ["TIER 1 · VOLUME", "portfolio size / share"]);
...
slide.addText([
  ...
  { text: "Volume:  disb run rate · acceptance & conversion · active partners · CL utilization", options: { color: BLUE } },
...
slide.addText([
  ...
  { text: "Star & Question marks → push volume (all PMs sit here). Cash cow (Offline) → maintain, no PM. Low profitability is acceptable today where the phase says grow the size." },
...
slide.addText("BASELINE — MAY 2026 READ", {
  x: c1x, y, w: c1w, h: 0.2, fontFace: F, fontSize: 8.5, bold: true, color: INK, margin: 0, charSpacing: 0.5,
});
y += 0.22;
slide.addText(
  "Overall PBT slightly negative. QRPh small positive · Online possibly positive, small · Moto calculated negative, long run unknown · Credit Line launched 15 Jul · Solar kill-or-keep pending. The H2 job: grow the stars and question marks into the absolute number.",
  { x: c1x, y, w: c1w, h: 0.95, fontFace: F, fontSize: 8.5, color: "3D3D3D", margin: 0, lineSpacingMultiple: 1.14, valign: "top" }
);
...
const owners = [
  ...
  ["Anton Betia — Deals & Solar", "Deals P25 → 50 → 100M/mo staged, at positive NPM  ·  acceptance & conversion (salvage plan on rejects)"],
  ["Manzil Balani — Deals BD", "Managed-account disb (ProTech · Maxicare · 3Cat)  ·  new merchants live, ticket uplift P1,000–2,000"],
  ["Elias Marcella — Credit Line & Card", "CL utilization & drawdown (target set ~15 Aug)  ·  per-card break-even at CAC P300–500"],
  ["Jastin Lagumbay — QRPh & Onboarding", "QRPh P107M/mo run rate  ·  onboarding cycle time signed → live, zero config errors"],
];
...
slide.addText([
  ...
  { text: "agreed roadmap steps delivered on pace — scored weekly in the tracker.   ", options: {} },
...
slide.addText([
  ...
  { text: "e.g. salvage ~20% of Deals rejects: at ~5% conversion that can triple volume — that estimate, on a date, becomes the month's number.", options: { italic: true, color: "3D3D3D", breakLine: true } },
...
y = 1.06;
secLabel(c3x, y, c3w, "4 · TRACKING RHYTHM — STRICT FROM JUL"); y += 0.3;
const cadence = [
  ["Weekly · Mon standup", "Exec tracker: status, due vs delivered, blockers, impact. Detail stays in ClickUp (linked per row)."],
  ["Monthly · KPI readout", "Target vs actual per line — Nhat's PBT model + Jenny's dashboard."],
  ["Quarterly · scorecard (first: end Sep)", "Revolut-style review per PM: Culture · Skills · Deliverables = (Speed + Quality) × Complexity, vs seniority talent bar → A-player / Above bar / Underperformer. A-players 15–25%, calibrated."],
  ["Semi-annual · promotion & pay", "Promotions on sustained A-player grades; pay to benchmark; bonus multiplier = grade × KPI attainment."],
];
cadence.forEach(([h, b]) => {
...
slide.addText([
  ...
  { text: "2.  Volume targets: QRPh 107 · Moto 15 · Deals ramp timing · CL utilization target on 15 Aug", options: { breakLine: true } },
  { text: "3.  Adopt the PO scorecard + talent bar for the 5 PMs; first review end of Sep", options: { breakLine: true } },
  { text: "4.  Bonus linkage: scorecard grade × KPI attainment, team and lead", options: {} },
], {
...
// ---------- footer ----------
slide.addText(
  "Working docs: Executive Project Tracker v2 (weekly, linked to the KPI tree, ClickUp links per row)  ·  KPI Targets H2 tab (monthly)  ·  QuantumLight Product Owner scorecard (quarterly).",
